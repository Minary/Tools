namespace HttpReverseProxy.Plugin.SslStrip.Cache
{
  ...
  public class CacheRedirect
  {
    ...
    public void AddElement(string id, string keyLocation, string valueLocation)
    {
      ...
      if (NeedsRequestBeMapped(keyLocation))
      {
        throw new Exception("Key was already added to the cache");
      }

      Logging.Instance.LogMessage(id, Logging.Level.DEBUG, "CacheRedirect.Cache.AddElement() : {0} => {1}", keyLocation, valueLocation);
    ...
    public void EnumerateCache()
    {
      ...
      {
        Logging.Instance.LogMessage("SslStrip.CacheRedirect", Logging.Level.DEBUG, "SslStrip.CacheRedirect.EnumerateCache() : Key:{0} Value:{1}, Counter:{2}", tmpKey, this.cache[tmpKey].Url, cache[tmpKey].Counter);
      }
    }
