namespace HttpReverseProxy.Plugin.ClientRequestSniffer
{
  ...
  public partial class ClientRequestSniffer : IPlugin
  {
    ...
    private void SendClientRequestDataToPipe(RequestObj requestObj)
    {
      ...
      this.pluginProperties.PluginHost.LoggingInst.LogMessage(requestObj.Id, HttpReverseProxyLib.Logging.Level.DEBUG, "ClientRequestSniffer.SendClientRequestDataToPipe(): Sending data to attack service pipe: {0} ...", pipeData.Trim().Substring(0, 40));
    }
    ...
    public bool WriteToPipe(RequestObj requestObj, string pipeData)
    {
      ...
      {
        this.pluginProperties.PluginHost.LoggingInst.LogMessage(requestObj.Id, HttpReverseProxyLib.Logging.Level.WARNING, "ClientRequestSniffer.WriteToPipe(TimeOutException): {0}", tex.Message);
      }
      catch (Exception ex)
      {
        this.pluginProperties.PluginHost.LoggingInst.LogMessage(requestObj.Id, HttpReverseProxyLib.Logging.Level.WARNING, "ClientRequestSniffer.WriteToPipe(Exception): {0}", ex.Message);
      }
