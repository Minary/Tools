...
DWORD WINAPI PacketHandlerArpMitm(LPVOID lpParam)
{
  ...
  {
    LogMsg(DBG_ERROR, "PacketHandlerArpMitm(): Unable to open the adapter");
    retVal = 5;
    goto END;
  }
  ...
  {
    LogMsg(DBG_ERROR, "PacketHandlerArpMitm(): Unable to compile the BPF filter \"%s\"", filter);
    retVal = 6;
    goto END;
  }
  ...
  {
    LogMsg(DBG_ERROR, "PacketHandlerArpMitm(): Unable to set the BPF filter \"%s\"", filter);
    retVal = 7;
    goto END;
  }

  LogMsg(DBG_INFO, "PacketHandlerArpMitm(): Enter listening/forwarding loop.");
  ...
  if (funcRetVal < 0)
  {
    ...
    LogMsg(DBG_ERROR, "PacketHandlerArpMitm(): Listener stopped unexpectedly with return value: %d, %s", funcRetVal, errorMsg);
  }
  else
  {
    LogMsg(DBG_INFO, "PacketHandlerArpMitm(): Listener stopped regularly with return value: %d", funcRetVal);
  }
