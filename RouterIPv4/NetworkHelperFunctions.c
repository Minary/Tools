...
void MacBin2String(unsigned char macAddrParam[BIN_MAC_LEN], unsigned char *outputParam, int outputLengthParam)
{
  if (outputParam && 
      outputLengthParam > 0 && 
      macAddrParam != NULL && 
      outputLengthParam >= MAX_MAC_LEN)
  {
    snprintf((char *)outputParam, outputLengthParam, "%02hhX-%02hhX-%02hhX-%02hhX-%02hhX-%02hhX", macAddrParam[0], macAddrParam[1], macAddrParam[2], macAddrParam[3], macAddrParam[4], macAddrParam[5]);
      }
}
